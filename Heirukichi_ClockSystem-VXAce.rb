...
# DESCRIPTION
#------------------------------------------------------------------------------
# This script adds a day/night cycle. In addition to that it adds a clock
# sprite so players can get a grasp on game time.
#==============================================================================
# INSTRUCTIONS
#------------------------------------------------------------------------------
# To set up this script you can change values inside HCK::CONFIG module. The
# purpose of each of those values is explained in detail there. If you just
# want your day/night cycle there are few default values you can use for a fast
# plug-and-play use of this script.
#
# This script is meant to work with two clock images. One is fixed (it is
# called clock support or just support in the script) while one can move (that
# is called just clock). The default path for those images is inside
# Graphics/Clock/ but feel free to change that when configuring the script.
#
# Those two images must have the same size and are displayed one above the
# other. Which one goes above and which one goes below is up to you. In the
# settings you have a value for that too.
# 
# NOTICE
# Since this script is meant as a night/day cycle, while it can still provide
# in-game time using $game_clock.to_s (it uses the format hh:mm AM/PM), the
# sprite moves over a cycle of 24 hours (not 12).
# For this reason, it is suited for something showing at which point of the
# cycle you are but you cannot use it to make a real clock (unless it is a
# digital one, in which case it works).
#
# While inside the engine you can use $game_clock.set_time(phase) in a script
# call to change your in-game time withuot having to wait that time actually
# passes by. You can use it to let the player rest in an inn or in any other
# situation that requires your time to flash-forward a bit. In this script call
# "phase" is the 30 minutes interval you want for your time.
#
# Examples:
...
#
# NOTICE
# The maximum value for phase is 47 while its minimum is 0. Any other value
# will be converted in a value between 0 and 47 automatically.
#
# If you decide to not initialize your clock at the beginning of the game and
# you want to set it up later using an event you can use
#
# $game_clock.setup(display)
#
# This will set up your clock and makes it ready to work.
# Its visibility depends on display value. It can be either true or false. If
# display is omitted then the default value (true) is used. Using setup
# automatically sets your time to HCK::CONFIG::STARTING_PHASE.
# No $game_clock.set_time needed after a setup.
#
# Once your clock has been set up you can access many different methods using
# script calls.
#------------------------------------------------------------------------------
# METHODS YOU CAN USE TO CONTROL YOUR CLOCK
#------------------------------------------------------------------------------
# - $game_clock.toggle
#     makes your clock visible when invisible or hides it if visible.
# - $game_clock.show
#     forces your clock to be visible. It has no effect when already visible.
# - $game_clock.hide
#     forces your clock to disappear. If already invisible it has no effect.
# - $game_clock.stop
#     stops your in-game time.
# - $game_clock.resume
#     resumes your in-game time.
# - $game_clock.toggle_timeflow
#     can either stop your clock or resume it depending on its status.
# - $game_clock.to_s
#     returns a string containing your in-game time in the format hh:mm AM/PM.
# - $game_clock.visible?
#     tells you if your clock is visible.
# - $game_clock.hour
#     returns your in-game hour in 12h format.
# - $game_clock.hour24
#     returns your in-game hour in 24h format.
# - $game_clock.minute
#     returns your in-game minutes.
# - $game_clock.phase
#     returns the current 30 minutes interval.
# - $game_clock.force_refresh
#     forces a sprite refresh for your clock.
# - $game_clock.set_indoor(value) 
#     tells the engine if you are inside or outside. Value is true when inside
#     and false when outside. This is used to prevent the engine from tinting
#     your screen automatically. Clock indoor flag is set to false when
#     initializing $game_clock. If you want it to be true do not forget to do
#     it manually! You can do it before setting up your clock so that no screen
#     tone is accidentally applied.
# - $game_clock.toggle_indoor
#     toggles indoor flag setting it to true if false or vice versa.
# - $game_clock.indoor?
#     returns true if your indor flag is true, false otherwise.
# - $game_clock.day? $game_clock.night? $game_clock.dawn? $game_clock.dusk?
#     return true if the chosen phase is running, false otherwise.
#------------------------------------------------------------------------------
# GAME INTERPRETER CHANGES
#------------------------------------------------------------------------------
# This script changes how Tint Screen works and whenever you want to tint your
# screen back to normal it automatically detects the color of your current
# phase of the day and uses that one instead. For this reason I added a few
# methods to tint your screen to the normal (day) color using a script call
# even when it is using a different color.
#
...
# - tint_normal(duration) tints your screen to the day phase color (clock
#   opacity goes to 255).
#
# duration is the amount of frames your titn screen takes to complete. It can
# be omitted. Default value when omitted is 30 (same as Fadeout and Fadein).
#
# NOTICE
# Tint Screen is NOT Fadeout nor Fadein! Tint Screen does not let your message
# disappear nor tints your pictures.
#
# NOTICE
# Game_Interpreter methods are not Game_Clock methods. You can use them in a
# script call by just writing them as they are. Using $game_clock.tint_black
# because it DOES NOT WORK!
#==============================================================================
# COMPATIBILITY
#------------------------------------------------------------------------------
# This script creates few new classes and create aliased methods for existing
# ones. Since there are no overwritten methods in this script it should
# (theoretically) be compatible with any other script overwriting one of those
# methods as long as this one is placed BELOW it.
#------------------------------------------------------------------------------
...
#   The script is now under the GNU Genreal Public License v3.0. It does not
#   affect the way you can use the script, it only changes how you should
#   credit me (a link to my website page for this script is mandatory now).
#   If you are using a previous version of this script, you are not required to
#   provide a link to my website, althoughit is much appreciated.
#------------------------------------------------------------------------------
# Version 1.0.4 [05-02-2019]
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# * $game_clock.set_time now has an internal check to convert numbers into the
#   0-47 range. It is now possible to add extra phases to the current time
#   using set_time, without having to worry about the new value being less than
#   47.
#
# Example:
#  $game_clock.phase # -> 36
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# * Fixed a bug that caused screen tint to be applied regardless of
#   TINT_ON_CHANGES value.
#------------------------------------------------------------------------------
# Version 1.0.2 [03-13-2019]
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
# * It is now possible to configure your clock so that it only updates when
#   $game_player is moving. This was meant to stop time flow in turn based
#   strategy games when the player is not moving.
#==============================================================================
...
module HCK
  ...
  module CONFIG
    ...
    #==========================================================================
    # Change the following value to be the distance between your clock and
    # window border. Default is 40.
    #==========================================================================
    ...
    #==========================================================================
    # This one is your clock image resolution. Default is "48x48". You can
    # create images with different resolution. Just remember that the
    # resolution must be the same for both clock and support. Resolution must
    # be in the "WidthxHeight" format.
    #==========================================================================
    
    RESOLUTION = "48x48"
    
    #==========================================================================
    # Your clock is made using two different images. CLOCK is the image
    # rotating while SUPPORT is the structure containing that image. PATH is
    # your images directory path. Default PATH is "Graphics/Clock/". While
    # there is a default value for CLOCK and SUPPORT, feel free to change that
    # as much as you want as long as it contains the file you need.
    ...
    #==========================================================================
    # This flag is used to determine whether Clock is displayed above Support
    # or vice versa.
    ...
    #==========================================================================
    # The following value tells the engine the minimum rotation (in degrees)
    # needed to refresh your clock. Default is 2.
    #==========================================================================
    
    REFRESH_DEGREES = 2
    
    #==========================================================================
    # Change the following value to be the amount of frames in a single in-game
    # hour.
    ...
    #==========================================================================
    # This flag is used to determine if your clock will be visible since the
    # beginning of the game. Default is true. If you want to display your clock
    # starting from a certain point of your game instead of displaying it at
    # the beginning of the game just set it to false and then setup your clock
    # somewhere else in an event. To set up your clock you can use the
    # following script call:
    #
    # $game_clock.setup
    #
    # The clock will NOT let you set it up again once it has already been set
    # up. If you want to change your in-game time you can set its time later.
    #==========================================================================
    
    CLOCK_ON_START = true
    
    #==========================================================================
    # Change this to be the starting phase (30 minutes interval) for your game.
    # Default is 16 (8 AM).
    #==========================================================================
    ...
    #==========================================================================
    # This value can be either true or false. When true your screen is tinted
    # according to the phase of the day you are in (Night, Dawn, etc.). Default
    # is true. If set to false, do not bother with the next two sets of values.
    #==========================================================================
    
    TINT_ON_CHANGES = true
    
    #==========================================================================
    # The following values define your screen tone. You can use them to tint
    # your screen as time passes during the day. Values are as follows:
    # DAY_PHASE = [Red, Green, Blue, Gray, StartingPhase, EndingPhase]
    # StartingPhase is the value of your clock phase when your screen start
    # being tinted with that color, Ending phase is the last one.
    #==========================================================================
    ...
    #==========================================================================
    # Change this to be your phase transition length (in frames). Default is
    # 300. Every value higher than 300 will be 300 by default. This is not
    # something I want, but it is something the library itself does not allow.
    #==========================================================================
    
    PHASE_TRANSITION = 300
    
    #==========================================================================
    # These parameters are used to determine if your clock can be update when
    # moving or not.
    # Set either UPDATE_WHEN_MOVING or UPDATE_WHEN_MOVING_WITH_SWITCH to true
    # if you want your clock to update only when moving. The main difference
    # between the two is that if UPDATE_WHEN_MOVING_WITH_SWITCH is set to true
    # and the switch is set to false, the clock updates normally. To reserve a
    # switch set UPDATE_WHEN_MOVING_SWITCH to be your switch ID. Default is 1.
    # If you do not want to reserve a switch for such purpose, but you still
    # want your clock to only update when moving, set UPDATE_WHEN_MOVING to
    # true. UPDATE_WHEN_MOVING and UPDATE_WHEN_MOVING_WITH_SWITCH are both
    # false by default.
    #==========================================================================
  ...
  #============================================================================
  # The following module contains a few methods used to debug this script. Set
  # ACTIVE to true to check if you configured this script correctly. Set it to
  # false otherwise.
  #============================================================================
  ...
  #============================================================================
  # Get number of frames in one hour. Much shorter if DEBUG::ACTIVE is true.
  #============================================================================
  ...
  #============================================================================
  # Get number of frames in one minute. Much shorter if DEBUG::ACTIVE is true.
  #============================================================================
...
module DataManager
  ...
  #============================================================================
  # This part is used to alias a few methods inside DataManager module. Those
  # methods are used to export $game_clock into save files and load it from
  # saved games.
  #============================================================================
...
class Sprite_Clock
  ...
  def update_visibility
    fade($game_clock.fade_speed) if ($game_clock.visible? && $game_clock.fading?)
    @clock.visible = visible
    @support.visible = visible
  end
...
class Game_Clock
  ...
  #============================================================================
  # Determine phase. Each day has 48 phases (30 minutes interval). Used for
  # purposes like transition between bright and dark and similar stuff.
  #============================================================================
  ...
  #============================================================================
  # This method is used when tinting screen or when fading out. It tells the
  # engine to change clock opacity while changing screen color.
  #============================================================================
